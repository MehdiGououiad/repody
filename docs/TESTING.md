...
6. **Agent recipe** — `resolve_recipe` unit tests plus the pool name map in `runtime/pools.py`.
7. **Hot path seams** — claim / finalize have Postgres integration tests (no workers required).
...
| Agent recipe | ✓ resolve_recipe | finalize helpers | IDP only |
...
| Taskiq / outbox | settings + input | ✓ outbox retry | live drain |
