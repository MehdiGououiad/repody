...
| **Service split** | Three images: `audit-api`, `audit-worker`, `audit-web` | Control / worker / edge scale independently; shared Python monorepo for now |
| **Inference** | External vLLM (out of chart) | GPU workloads on separate node pool or managed inference |
| **Observability** | Loki/Grafana optional (Compose); Sentry/GlitchTip via env DSN in K8s | Hybrid model from ADR observability work |
| **Compose path** | `compose.microservices.yaml` on prod stacks | Same images locally before K8s push |
...
- Helm chart under `deploy/helm/audit-platform/` is the production K8s entrypoint.
- `pnpm platform:images:build` / `platform:images:push` produce registry tags referenced in `values.yaml`.
