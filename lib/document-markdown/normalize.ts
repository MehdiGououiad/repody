/** Client-side NuExtract markdown cleanup (mirrors backend markdown_normalize.py). */
...
export function formatPlainMarkdownLines(lines: string[]): string {
  const blocks: string[] = [];
...
function formatPlainDocumentMarkdown(text: string): string {
  if (looksLikeStructuredMarkdown(text)) return text;

  const headers = text.match(PAGE_HEADER_RE) ?? [];
  if (!headers.length) {
    return formatPlainMarkdownLines(text.split("\n"));
  }
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    const cleaned = formatPlainMarkdownLines(parts[index].split("\n"));
...
export function normalizeDocumentMarkdown(text: string): string {
  ...
  normalized = formatPlainDocumentMarkdown(normalized);
