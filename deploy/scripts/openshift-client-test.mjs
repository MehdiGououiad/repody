...
const PROMOTED_VALUE_PATHS = [
  "deploy/client/lab/values.openshift-local.promoted.yaml",
  "deploy/client/lab/values.auth.openshift-local.promoted.yaml",
];

/** GitOps: register writes cluster-specific values; push so Argo pulls the same revision. */
function commitPromotedValuesIfChanged() {
  if (dryRun) return;
  const status = spawnSync("git", ["status", "--porcelain", ...PROMOTED_VALUE_PATHS], {
    cwd: root,
    encoding: "utf8",
  });
  if (!(status.stdout ?? "").trim()) {
    log("promote", "promoted values unchanged");
    return;
  }
  const add = spawnSync("git", ["add", ...PROMOTED_VALUE_PATHS], { cwd: root });
  if (add.status !== 0) fail("git add promoted values failed");
  const commit = spawnSync(
    "git",
    [
      "commit",
      "-m",
      "chore(lab): update promoted GitOps values from register",
      "-m",
      "Auto-generated by openshift-client-test.mjs for cluster-specific hosts and registry.",
    ],
    { cwd: root },
  );
  if (commit.status !== 0) fail("git commit promoted values failed");
  const push = spawnSync("git", ["push", "origin", "HEAD"], { cwd: root });
  if (push.status !== 0) {
    fail("git push promoted values failed — Argo CD syncs remote master, not local files only");
  }
  log("promote", "promoted values committed and pushed");
}

function waitRepodyWorkloads(timeoutSec) {
  if (dryRun) return;
  const workloads =
    profile === "external"
      ? [
          ["deployment", "repody-auth-keycloak"],
          ["deployment", "repody-api"],
          ["deployment", "repody-web"],
        ]
      : [
          ["statefulset", "repody-data-postgresql"],
          ["statefulset", "repody-data-redis-master"],
          ["statefulset", "repody-data-minio"],
          ["deployment", "repody-auth-keycloak"],
          ["deployment", "repody-api"],
          ["deployment", "repody-web"],
        ];
  for (const [kind, name] of workloads) {
    const result = kube.kubectl(
      ["rollout", "status", `${kind}/${name}`, "-n", NS.repody, `--timeout=${timeoutSec}s`],
      { quiet: true },
    );
    if (result.status !== 0) {
      fail(`${kind}/${name} did not become ready within ${timeoutSec}s`);
    }
  }
  log("sync", "workloads ready (kubectl rollout status)");
}
...
function sync() {
  ...
  if (!skipSyncWait) {
    const timeoutSec = fast ? 180 : Number(process.env.REPODY_SYNC_WAIT_SEC || 600);
    waitRepodyWorkloads(timeoutSec);
  } else {
    log("sync", "skipped workload wait (--skip-sync-wait or --fast); verify checks readiness");
  }
  endStep("sync");
}
