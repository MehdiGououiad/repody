...
def format_plain_ocr_lines(lines: Iterable[str]) -> str:
    """Turn OCR text lines into markdown-friendly paragraphs for preview."""
    blocks: list[str] = []
    current: list[str] = []
...
def _format_plain_ocr_markdown(text: str) -> str:
    ...
    if not headers:
        return format_plain_ocr_lines(text.splitlines())

    sections: list[str] = []
    for index, body in enumerate(parts):
        if index < len(headers):
            sections.append(headers[index])
        cleaned = format_plain_ocr_lines(body.splitlines())
        if cleaned:
...
def normalize_ocr_markdown(text: str | None) -> str | None:
    ...
    normalized = _format_plain_ocr_markdown(normalized)
