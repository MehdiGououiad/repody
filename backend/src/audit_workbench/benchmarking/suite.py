...
from audit_workbench.benchmarking.text import normalize_space, parse_amount
...
def compare_value(
    ...
    *,
    ...
    if comparison == "amount":
        actual_amount = parse_amount(actual)
        expected_amount = parse_amount(expected)
        tolerance_amount = parse_amount(tolerance) or Decimal("0")
        return (
