...
async def test_keycloak_user_realm_roles_accepts_keycloak_list_response(
    ...
    monkeypatch.setattr(keycloak, "_admin_request", fake_admin_request)

    roles = await keycloak.user_realm_roles("user-1")

    assert [role["name"] for role in roles.unwrap()] == ["platform_admin", "viewer"]
