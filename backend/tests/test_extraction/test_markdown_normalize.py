from audit_workbench.extraction.markdown_normalize import (
    format_plain_markdown_lines,
...
def test_html_table_to_markdown():
    ...
    """
    out = normalize_document_markdown(html)
...
def test_runon_totals_split():
    ...
    out = normalize_document_markdown(raw)
...
def test_plain_markdown_lines_use_paragraph_breaks():
    out = format_plain_markdown_lines(["Invoice number: INV-42", "Total TTC: 6000,00 MAD"])
    assert "**Invoice number:** INV-42" in out
    assert "**Total TTC:** 6000,00 MAD" in out
    assert "\n\n" in out


def test_plain_markdown_page_sections_keep_headers():
