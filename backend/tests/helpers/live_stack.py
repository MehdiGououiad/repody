"""Helpers for pytest live tests against a running Kubernetes stack."""
...
def fetch_keycloak_token() -> str:
    response = httpx.post(
        f"{live_auth_url()}/realms/repody/protocol/openid-connect/token",
        data={
            "grant_type": "password",
...
def create_anonymous_live_client(**kwargs) -> httpx.Client:
    """HTTP client without OIDC bearer — for workflow API-key auth checks."""
    return httpx.Client(
        base_url=live_api_base(),
        timeout=kwargs.pop("timeout", 30.0),
        **kwargs,
...
def assert_metrics_access(response: httpx.Response, *, oidc_enabled: bool) -> None:
    """Operator role lacks metrics:read when OIDC RBAC is active."""
    if oidc_enabled:
...
def assert_settings_config_access(response: httpx.Response, *, oidc_enabled: bool) -> None:
    """Operator role lacks settings:read when OIDC RBAC is active."""
    if oidc_enabled:
        assert response.status_code == 403
    else:
        assert response.status_code == 200
